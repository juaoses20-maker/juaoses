...
- Landing (2026-09-16): nueva sección "En el campo" (`field.*` en messages) entre "Qué hace por ti" y "La app por dentro" — 3 fotos reales de trabajo de campo del usuario (`public/landing/campo-{1,2,3}.jpg`): mapa de ruta medida, cable saliendo del carrete, perforación dirigida (HDD). Refuerza credibilidad visual sin tocar el resto de la composición/paleta ya aprobada.
...
- **veredicto:pantalla-principal PENDIENTE** — `/app` ("Hoy") está construida y en uso real por el usuario, pero no ha pasado por el subagente `revisor-visual` todavía. Se hará junto con landing/onboarding/paywall en el Bloque 6 (testing + pulido), no antes.
