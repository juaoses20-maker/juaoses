...
- [x] **App interna conectada a datos reales** — 2026-09-08. `lib/data/`: `types.ts` (compartido), `queries.ts` (server-only), `actions.ts` (Server Actions). `lib/plan-upload.ts` + `lib/photo-upload.ts` (subida cliente con compresión + geolocalización).
  ...
  - **Planos** `/app/planos` + `/app/planos/[id]`: subir PDF o foto a `plans/…`. Editor estilo FiberField (ref del usuario 2026-09-09): **pincel multi-trazo** (varios trazos/ramales/colores por ruta → barra "N trazos" → un solo formulario: actividad + pies totales + cuadrilla → marca `{t:strokes}`), **zoom +/−/ajustar**, **pan**, **marcadores con etiqueta** (M/L/T), **notas** sobre el plano. `geom` `{t:strokes|path|seg|pt|text}` compatible con filas viejas. `pointer capture` en el lienzo. PDF: se guarda y se abre; dibujo encima requiere subirlo como foto. Pendiente: rasterizar páginas PDF (pdfjs) + rotar.
