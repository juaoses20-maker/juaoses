...
export default function PlanosUI({
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    e.target.value = "";
    if (!file) return;
    const name = (file.name.replace(/\.[^.]+$/, "") || t("defaultName")).slice(0, 60);
    setUploading(true);
    setErr(null);
    try {
    ...
    <div className="relative">
      <div className="mt-2 flex flex-col">
        ...
        <p className="text-[10px] text-ink-3">
          {plans.length === 0 ? projectName : t("subtitle", { count: plans.length, project: projectName })}
        </p>
      </div>
          ...
          <div key={p.id} className="flex gap-3 rounded-[12px] border bg-surface p-2.5 shadow-[var(--shadow-1)]">
            ...
            <Link href={`/app/planos/${p.id}`} className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-[10.5px] text-ink-2">
                {t("marksLine", { count: p.mark_count, ft: nf.format(p.ft_marked) })}
              </div>
